...
import { Trophy, AlertCircle, Settings as SettingsIcon } from 'lucide-react';
...
export default function CoachSettings() {
  ...
  const toggleSport = async (sport: Sport) => {
    ...
    try {
      ...
      if (isSelected) {
        // Check if there are any athletes associated with this sport
        ...
        if (athletes && athletes.length > 0) {
          throw new Error('Cannot remove sport with active athletes');
        }
  ...
  const handleSettingToggle = async (setting: keyof TeamSettings) => {
    if (!user?.id || isSettingsUpdating) return;
    ...
    <div className="relative space-y-8">
      ...
      >
        ...
        <p className="text-gray-400 mt-2">
          Manage your sports and team settings
        </p>
      </motion.div>
      ...
      <Card className="relative bg-black/40 backdrop-blur-xl border border-blue-500/20">
        ...
        <CardContent className="space-y-4">
          {Object.entries(teamSettings).map(([key, value]) => (
            <div 
              key={key}
            ...
            >
              ...
              <Button
                ...
                onClick={() => handleSettingToggle(key as keyof TeamSettings)}
      ...
      {confirmationState.isOpen && confirmationState.sport && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <Card className="w-full max-w-md bg-black/90 border border-blue-500/20 text-white">
            ...
            <CardContent>
              <p>Are you sure you want to remove {confirmationState.sport.name}? This will not affect your existing athletes.</p>
